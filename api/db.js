...
function getUserProjects(userId) {
  const rows = query(
    'SELECT id, name, mode, description, thumbnail, is_public, published, published_at, share_token, created_at, updated_at, data FROM projects WHERE user_id = ? ORDER BY updated_at DESC',
    [userId]
  );
  return rows.map((r) => {
    let pageCount = 1;
