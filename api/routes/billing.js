...
router.get('/weekly', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    // 预览同样带上 PayFlow 权威口径(与生成的周报一致)
    const commerce = require('../lib/commerce');
    if (commerce.isDelegated()) {
      try {
        const pf = await require('../lib/payflow').request('GET', '/api/v1/analytics/summary', null, 'days=7');
        current.payflow = (pf && pf.summary) || null;
      } catch (e) {}
    }
...
router.post('/weekly/run', authMiddleware, adminOnly, async (req, res) => {
  try {
    const r = await weeklyReport.generate((req.body && req.body.period) || undefined);
