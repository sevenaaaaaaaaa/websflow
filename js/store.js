...
(function (WF) {
  ...
  WF.ensurePages = function (proj) {
    ...
    if (!Array.isArray(proj.segments)) proj.segments = [];   // 项目级分群
    if (!Array.isArray(proj.automations)) proj.automations = []; // 页面侧自动化规则
    return proj;
  };
  ...
  // ---------- 分群(项目级) ----------
  WF.addSegment = function (proj, name, rules) {
    WF.ensurePages(proj);
    const sg = { id: "sg" + Math.random().toString(36).slice(2, 7), name: name || "新分群", rules: rules || {} };
    proj.segments.push(sg);
    persist();
    return sg;
  };
  WF.updateSegment = function (proj, id, patch) {
    const sg = (proj.segments || []).find((x) => x.id === id);
    if (!sg) return null;
    Object.assign(sg, patch);
    if (patch && patch.rules) sg.rules = Object.assign({}, sg.rules, patch.rules);
    persist();
    return sg;
  };
  WF.removeSegment = function (proj, id) {
    proj.segments = (proj.segments || []).filter((x) => x.id !== id);
    proj.blocks.forEach((b) => {
      if (b.audience && b.audience.segmentId === id) delete b.audience.segmentId;
      if (b.personalize) b.personalize = b.personalize.filter((p) => p.segmentId !== id);
    });
    persist();
  ...
  // ---------- 页面自动化(触发-动作) ----------
  WF.addAutomation = function (proj, rule) {
    WF.ensurePages(proj);
    const r = Object.assign({
      id: "at" + Math.random().toString(36).slice(2, 7),
      name: "新规则", enabled: true,
      when: { type: "time_on_page", value: 30 },
      then: [{ type: "bar", text: "限时优惠进行中", linkText: "立即查看", link: "#" }],
    }, rule || {});
    proj.automations.push(r);
    persist();
    return r;
  };
  WF.updateAutomation = function (proj, id, patch) {
    const r = (proj.automations || []).find((x) => x.id === id);
    if (!r) return null;
    Object.assign(r, patch);
    if (patch && patch.when) r.when = Object.assign({}, r.when, patch.when);
    persist();
    return r;
  };
  WF.removeAutomation = function (proj, id) {
    proj.automations = (proj.automations || []).filter((x) => x.id !== id);
    persist();
  ...
  // 页面自带的新模块(生产流水线现场生成/自造模块)→ 载入时注册,保证编辑器能渲染与编辑
  WF.registerProjectModules = function (data) {
    try {
      const own = (data && data.customModules) || [];
      if (own.length && WF.registerPluginBlocks) WF.registerPluginBlocks(own);
    } catch (e) {}
    return data;
  };
  ...
  WF.importProject = function (data) {
    ...
    WF.registerProjectModules(data);
    const id = "p" + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
    const proj = {
      id,
  ...
  WF.normalizeBlock = function (b) {
    ...
    if (Array.isArray(b.personalize) && b.personalize.length) nb.personalize = JSON.parse(JSON.stringify(b.personalize));
    return nb;
  };
