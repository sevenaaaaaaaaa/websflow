...
(function (WF) {
    ...
    (WF.apiGetPlugins ? WF.apiGetPlugins() : Promise.reject())
      .then((list) => {
        ...
        if (usable.length) {
          ...
          // 页面自带的新模块(生产流水线/自造模块)
          try {
            const own = (WF.project && WF.project.data && WF.project.data.customModules) || [];
            if (own.length) WF.registerPluginBlocks(own);
          } catch (e) {}
  ...
  function segmentsHTML() {
    ...
    const plan = (WF.apiGetStoredUser() || {}).plan || "free";
    const upgrade = plan === "free" ? `<div class="ed-hint" style="margin-bottom:10px">${WF.t("segUpgradeHint")}
      <button class="ed-btn is-primary" style="margin-top:6px;width:100%" data-act="upgrade-pro">${WF.t("segUpgradeBtn")}</button></div>` : "";
    return `<div class="lib-cat__title">${WF.t("segmentsTitle")}</div>${upgrade}
  ...
  function onClick(e) {
    ...
    const actions = {
      ...
      "upgrade-pro": () => { location.hash = "#/console/settings"; },
