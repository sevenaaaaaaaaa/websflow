...
.wf-ph {
  ...
  background:
    radial-gradient(120% 120% at 12% 10%, var(--wf-primary-soft), transparent 62%),
    linear-gradient(135deg, #f8fafc, #eef2ff);
  color: var(--wf-muted); font-weight: 600; letter-spacing: .05em; font-size: 13px;
}
.wf-ph::before {
  content: ""; position: absolute; inset: 0;
  background-image: linear-gradient(45deg, rgba(99,102,241,.06) 25%, transparent 25%, transparent 50%, rgba(99,102,241,.06) 50%, rgba(99,102,241,.06) 75%, transparent 75%);
  background-size: 24px 24px;
}
.wf-ph::after {
  content: attr(data-label); position: relative; z-index: 1;
  padding: 7px 14px; border-radius: 999px; background: rgba(255,255,255,.82);
  box-shadow: 0 1px 2px rgba(15,23,42,.08);
}

/* split hero 无配图时的产品可视化(避免空白占位) */
.b-hero__mock {
  width: 100%; aspect-ratio: 4 / 3; border-radius: calc(var(--wf-radius) * 1.25);
  background: #fff; border: 1px solid var(--wf-border); overflow: hidden;
  box-shadow: 0 20px 45px rgba(49,46,129,.18);
}
.b-hero__mock-bar { height: 34px; display: flex; align-items: center; gap: 6px; padding: 0 12px; background: #f8fafc; border-bottom: 1px solid var(--wf-border); }
.b-hero__mock-dot { width: 8px; height: 8px; border-radius: 50%; background: #cbd5e1; }
.b-hero__mock-url { flex: 1; height: 14px; margin-left: 8px; border-radius: 7px; background: #eef2f7; }
.b-hero__mock-body { padding: 18px; display: grid; gap: 10px; }
.b-hero__mock-line { height: 10px; border-radius: 6px; background: #e2e8f0; }
.b-hero__mock-line.is-title { height: 20px; width: 64%; background: #1e293b; }
.b-hero__mock-line.is-sub { width: 78%; }
.b-hero__mock-line.is-accent { width: 108px; height: 28px; border-radius: 999px; background: var(--wf-primary); margin-top: 4px; }
.b-hero__mock-cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-top: 6px; }
.b-hero__mock-card { height: 58px; border-radius: 10px; background: #f1f5f9; border: 1px solid var(--wf-border); }

/* ---------- 入场动画 ---------- */
/* 渐进增强:只有 JS 就绪(html.wf-anim)才隐藏待入场元素 —— 无 JS/爬虫时内容始终可见 */
html.wf-anim .wf-root [data-reveal] { opacity: 0; transform: translateY(26px); transition: opacity .7s ease, transform .7s cubic-bezier(.2,.7,.3,1); transition-delay: var(--wf-delay, 0ms); }
html.wf-anim .wf-root [data-reveal="left"] { transform: translateX(-32px); }
html.wf-anim .wf-root [data-reveal="right"] { transform: translateX(32px); }
html.wf-anim .wf-root [data-reveal="zoom"] { transform: scale(.92); }
html.wf-anim .wf-root [data-reveal="none"] { transform: none; }
.wf-root [data-reveal].is-in { opacity: 1 !important; transform: none !important; }
...
/* 深色底 hero:文字与徽标转浅色(配图/夜景渐变) */
.b-hero__inner.is-dark-text { color: #fff; }
.b-hero__inner.is-dark-text .b-hero__title { color: #fff; text-shadow: 0 2px 18px rgba(0,0,0,.28); }
.b-hero__inner.is-dark-text .b-hero__subtitle { color: rgba(255,255,255,.88); }
.b-hero__inner.is-dark-text .b-hero__badge { background: rgba(255,255,255,.14); border-color: rgba(255,255,255,.3); color: #fff; backdrop-filter: blur(6px); }
.b-hero.is-v-split .b-hero__inner { padding-top: 92px; padding-bottom: 92px; }
...
.b-hero__bg.is-image::after { content: ""; position: absolute; inset: 0; background: linear-gradient(180deg, rgba(8,12,26,.62), rgba(8,12,26,.32) 45%, rgba(8,12,26,.55)); }
